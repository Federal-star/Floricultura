(function () {
  ...
  function render(items) {
    const hasFilters = Boolean(searchQuery || categoryFilter.value);
    const emptyMessage = hasFilters ? 'Nenhum produto encontrado com os filtros informados.' : 'Nenhum produto cadastrado.';
  ...
  async function load() { feedback.textContent = 'Carregando...'; try { const result = await produtosService.list(page, 10, categoryFilter.value, searchQuery); pagination = result.pagination; render(result.items); document.querySelector('#page-indicator').textContent = `Página ${pagination.page} de ${Math.max(pagination.totalPages, 1)}`; document.querySelector('#previous-page').disabled = page <= 1; document.querySelector('#next-page').disabled = page >= pagination.totalPages; feedback.textContent = pagination.total ? `${pagination.total} produtos` : ''; } catch (error) { feedback.textContent = error.message; feedback.classList.add('is-error'); } }
  ...
  document.querySelector('#new-product').addEventListener('click', () => { if (canEdit) openModal(); }); document.querySelector('#close-modal').addEventListener('click', closeModal); modal.addEventListener('click', (event) => { if (event.target === modal) closeModal(); }); document.querySelector('#close-drawer').addEventListener('click', closeBotanicalCard); drawer.addEventListener('click', (event) => { if (event.target === drawer) closeBotanicalCard(); }); document.addEventListener('keydown', (event) => { if (event.key === 'Escape') { closeModal(); closeBotanicalCard(); } }); document.querySelector('#product-category').addEventListener('change', toggleBotanical); categoryFilter.addEventListener('change', () => { page = 1; load(); }); searchInput.addEventListener('input', () => { searchQuery = searchInput.value.trim(); page = 1; window.clearTimeout(searchDebounce); searchDebounce = window.setTimeout(load, 300); }); clearFiltersButton.addEventListener('click', () => { searchInput.value = ''; categoryFilter.value = ''; searchQuery = ''; page = 1; load(); searchInput.focus(); }); document.querySelector('#previous-page').addEventListener('click', () => { if (page > 1) { page -= 1; load(); } }); document.querySelector('#next-page').addEventListener('click', () => { if (page < pagination.totalPages) { page += 1; load(); } });
